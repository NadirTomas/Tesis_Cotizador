...
- Stock físico real: `StockSheet` (FULL_SHEET/REMNANT), `StockReservation`, `StockMovement`; flujo completo crear → recomendar → reservar → confirmar corte → generar retazo, con geometría real (Shapely, polígonos con huecos), idempotencia verificada bajo concurrencia REAL de Postgres (no solo teoría — ver §9). Permisos por acción, no simétricos: crear/editar/descartar chapa requiere OWNER; consultar, recomendar, reservar, liberar y confirmar corte están abiertos a cualquier rol — **excepto `GET /stock/{id}/movements` (historial de auditoría de la chapa), que también requiere OWNER** (decisión intencional confirmada 2026-09-14, no una inconsistencia: expone quién hizo cada acción, más apropiado a nivel de dueño que como dato operativo — ver `PROJECT_MEMORY.md` §5)
...
- [ ] `PUT /stock/{stock_id}` (solo permite corregir `material_id` de una chapa, OWNER-only) **sigue sin UI**. El hueco de integridad encontrado el 2026-09-14 (se podía ejecutar sobre una chapa `RESERVED`/`CONSUMED` sin ningún guard) **ya se corrigió ese mismo día**: ahora solo permite corregir el material mientras la chapa está `AVAILABLE` — `RESERVED`/`CONSUMED`/`DISCARDED` devuelven `409` (ver §5 de `PROJECT_MEMORY.md`). Falta solo la UI — si se agrega, exponerla en `StockDetailPage.tsx` limitada a `status === "AVAILABLE"` (el backend ya lo exige, la UI solo necesitaría ocultar la acción en los demás estados).
...
- **2026-09-15 (bugs de testing manual)**: reabierto el core (declarado feature-frozen el 2026-09-14) para corregir 5 hallazgos reales de una usuaria probando el sistema. Resumen: (1) **usuario dado de baja podía "entrar"**: el backend ya rechazaba correctamente el login de un usuario globalmente inactivo (`User.is_active`) y ya bloqueaba operaciones de un membership de empresa inactivo (`CompanyMember.is_active`, vía `get_current_company`) — el bug real era 100% frontend: `LoginPage.tsx` auto-seleccionaba la única empresa del usuario sin fijarse en `member_is_active`/`is_active`, dejando entrar a la UI a un empleado desactivado (que luego chocaba con 403 en cada pantalla). Corregido: solo se auto-selecciona/lista como elegible una empresa con membership y empresa activos; si no queda ninguna activa, se desloguea con un mensaje claro. `SelectCompanyPage` ahora marca "Acceso desactivado" y deshabilita esas filas. **Seguimiento del mismo día**: un repaso posterior detectó que el fix de frontend no alcanzaba — `get_current_user` nunca revalidaba `User.is_active` contra la DB en cada request, solo en `/auth/login`/`/auth/refresh`, así que un access_token emitido antes de una baja global seguía sirviendo para operar hasta que expiraba por su propio `exp`. Corregido: `get_current_user` ahora consulta la DB en cada request autenticado y corta con `401` si el usuario ya no existe o está inactivo (esto **no** es revocación de tokens — no hay blacklist ni versionado de JWT, el token sigue siendo criptográficamente válido — es simplemente revalidar el estado del usuario en cada uso). Ver `test_auth_inactive.py::test_globally_inactive_user_old_access_token_is_rejected_by_protected_endpoints`. (2) **Validaciones de cliente**: no había ninguna en backend. Se agregó `email` con formato real (`EmailStr`, cualquier dominio), `cuit_cuil`/`phone` siguen siendo **opcionales** (no hay requisito documentado que los exija) pero si se cargan deben tener formato válido (11 dígitos / al menos 6 dígitos sin letras), `name` no puede estar vacío ni superar 200 caracteres pero admite números/puntuación (nombres comerciales tipo "Metalúrgica 3H S.R.L."). (3) **Fecha de vencimiento de cotización**: se pudo reproducir el bug reportado (PDF con "01/05/1") — el `<input type="date">` de HTML5 permite tipear un año de 1 dígito y el backend no validaba nada. Se agregó: año mínimo 2000, y `due_date >= issue_date`. No se exigió `issue_date >= hoy` (deliberado: hay flujo legítimo de cargar cotizaciones acordadas días atrás; ver `PROJECT_MEMORY.md` para el detalle). (4) **Materiales**: sin validación de strings (vacío/espacios/control chars/longitud) ni de numéricos (thickness/sheet_width/sheet_height debían ser `> 0`, `sheet_cost_ars` debía ser `>= 0` — 0 es un valor de negocio real, usado en `test_quotation_calculator.py` para aislar el costo de máquina, así que **no** se exigió `> 0` ahí). Nombres técnicos con números/símbolos (`AISI 304`, `Acero 1/8"`) siguen aceptándose sin restricción de charset. (5) **`[object Object]` al crear empleado**: causa raíz encontrada — `services/companies.ts` (`createMember`/`updateMember`) eran los únicos servicios que no usaban el helper ya existente `parseErrorDetail` (creado específicamente para este problema, ver `apiClient.ts`), así que un 422 de Pydantic (`detail` como array de objetos) se convertía a texto crudo. Corregido usando el mismo helper. De paso se encontró y corrigió el mismo patrón en `clients.ts`/`materials.ts`/`quotations.ts` (necesario porque las nuevas validaciones de (2)-(4) iban a disparar 422 reales en uso normal, y esos formularios mostraban un mensaje genérico fijo en vez del real). **Commiteado localmente, sin push a `origin/main` ni deploy** — a la espera de que se decida reactivar producción.
- **2026-09-14 (auditoría final)**: cerrado el último P1 de integridad detectado en la auditoría final de contexto: `discard_stock_sheet` permitía descartar una chapa `RESERVED` (dejando su `StockReservation` `ACTIVE` huérfana, sin ningún camino de UI para liberarla) y no tenía protección de carrera — bajo timing real contra `confirm_cut` era posible sobrescribir `CONSUMED → DISCARDED` después de generar retazos. Corregido: solo se puede descartar desde `AVAILABLE` (mismo patrón de `UPDATE` condicional que `reserve_stock`/`confirm_cut`/`update_stock_sheet`), `RESERVED`/`CONSUMED`/`DISCARDED` devuelven `409`. Se eligió deliberadamente la Opción A (rechazar, no liberar la reserva automáticamente) para no darle a una operación administrativa efectos secundarios ocultos sobre una cotización activa — ver `PROJECT_MEMORY.md` §13 para el detalle completo y el razonamiento. Además se agregó confirmación en el frontend antes de cancelar una cotización (antes era una acción destructiva sin diálogo, inconsistente con eliminar ítem/descartar stock). **Commiteado localmente, sin push a `origin/main` ni deploy** — a la espera de que se decida reactivar producción.
...
- **2026-08-25 (mañana)**: se escribió el fix de `analyze_dxf` (sumaba área de agujeros en vez de restarla, círculos no contaban para área) — desalineaba el costo de material cotizado contra el área real descontada del stock. **El fix quedó sin commitear ese día** (ver `PROJECT_MEMORY.md` §20 de esa fecha) y recién se commiteó/deployó el **2026-08-26** (`134425d`). La afirmación previa de este documento de que se había aplicado un backfill a piezas existentes en producción ese mismo 25/08 era **incorrecta** — cronológicamente no pudo haber usado una fórmula que todavía no estaba deployada.
- **2026-09-14**: cerrado el P0 de la auditoría de contexto. Se implementó `backend/scripts/recalculate_piece_geometry.py` — script administrativo idempotente, dry-run por defecto, que reanaliza el `dxf_data` ya guardado con `analyze_dxf()` real y recalcula `QuotationItem`s en `draft` (nunca `sent`/`accepted`/`cancelled`, que conservan su snapshot de costos byte a byte) — commiteado en `a089991`. Se corrió el **dry-run real contra la PostgreSQL de producción** (vía `railway run`, sin `--apply`): **6 piezas con DXF inspeccionadas, las 6 ya tenían `area_mm2`/`length_cut_mm` correctos según la implementación actual, 0 piezas requerían corrección, 0 errores de DXF, 0 cotizaciones draft requerían recálculo**. Ninguna de las piezas reales en producción tiene agujeros internos ni entidades `CIRCLE` — el patrón geométrico que dispara el bug simplemente no está presente en los datos actuales. **Se verificó producción mediante dry-run y no se encontraron registros que requirieran corrección — por lo tanto no fue necesario ejecutar `--apply`.** Incidente cerrado como *verificado sin datos afectados en producción* (no se aplicó ningún backfill masivo, porque no hacía falta). El script queda en el repo como herramienta administrativa disponible para la próxima vez que se cargue una pieza real con geometría compleja, o para una futura auditoría/importación legacy — ver `PROJECT_MEMORY.md` §7 y §20 para el detalle completo, incluidas las 2 piezas legacy sin DXF que quedan fuera del alcance de este mecanismo.
...
- ~~`setup_time_min` por unidad vs. por corrida completa~~ — **resuelto 2026-09-14**, Cortesar confirmó "una sola vez por lote/trabajo". Fórmula corregida (ver §6). Cotizaciones históricas (`sent`/`accepted`/`cancelled`) quedan con su snapshot congelado, sin recalcular. El único draft existente en producción afectado (`COT-0008`/`item_id=19`) fue recalculado explícitamente con la función oficial ese mismo día; los demás drafts no tenían diferencia — ver `PROJECT_MEMORY.md` §6 para el detalle completo. 0 quotation items draft pendientes de la regla antigua, confirmado con un análisis de solo lectura posterior.
...
- No hay revocación de tokens JWT (blacklist/versionado) — decisión deliberada, no se va a implementar. Lo que sí existe (desde 2026-09-15): `get_current_user` revalida `User.is_active` contra la DB en cada request autenticado, así que una baja global corta el acceso de inmediato aunque el JWT en sí siga siendo válido y no haya expirado. La única ventana residual es cosmética: un token ya en curso en medio de un request individual no se corta a mitad de camino (no hay problema real, cada request es atómico).
