...
- **2026-08-25 (mañana)**: se escribió el fix de `analyze_dxf` (sumaba área de agujeros en vez de restarla, círculos no contaban para área) — desalineaba el costo de material cotizado contra el área real descontada del stock. **El fix quedó sin commitear ese día** (ver `PROJECT_MEMORY.md` §20 de esa fecha) y recién se commiteó/deployó el **2026-08-26** (`134425d`). La afirmación previa de este documento de que se había aplicado un backfill a piezas existentes en producción ese mismo 25/08 era **incorrecta** — cronológicamente no pudo haber usado una fórmula que todavía no estaba deployada. Corregido tras una auditoría de contexto el 2026-09-14: **el backfill de piezas históricas (creadas antes del 26/08 con agujeros o geometría circular) sigue pendiente de ejecutar en producción.** Diseño e implementación del backfill (`backend/scripts/recalculate_piece_geometry.py`, dry-run por defecto) ya están listos y testeados; falta correrlo contra la base real.
