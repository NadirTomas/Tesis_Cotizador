...
- No hay revocación de tokens JWT: un access_token emitido antes de desactivar globalmente a un usuario (`User.is_active=False`) sigue siendo válido para requests normales hasta que expira por su propio `exp` — solo `/auth/login` y `/auth/refresh` re-consultan `is_active`. Cerrar esto del todo requeriría una blacklist o versión de token (infraestructura nueva, fuera del alcance del fix de 2026-09-15).
