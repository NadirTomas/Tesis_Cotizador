...
**Stock — permisos**: crear/editar/descartar chapas requiere OWNER; consultar (listar/ver una chapa/ver reservas), pedir recomendación, reservar, liberar y confirmar corte están abiertos a cualquier rol de la empresa (uso del día a día). **Excepción explícita, decisión intencional confirmada 2026-09-14**: `GET /stock/{id}/movements` (el historial de auditoría append-only de una chapa) requiere OWNER, a diferencia del resto de los `GET` de stock. Motivo: `movements` expone quién (`created_by_id`) hizo qué acción y cuándo sobre cada chapa — es, en esencia, un log de auditoría de actividad de los empleados, y ese tipo de visibilidad se consideró más apropiada a nivel de dueño/gerencia que como dato operativo del día a día (a diferencia de `reserve`/`release`/`confirm-cut`, que sí son acciones operativas abiertas a cualquiera). No es una inconsistencia — se evaluó explícitamente y se decidió mantenerlo así; ver `PROJECT_CONTEXT.md` para el detalle.

**Regla de integridad, corregida 2026-09-14**: una chapa puede corregir su material únicamente mientras está `AVAILABLE`. Una vez reservada, consumida o descartada, su identidad de material forma parte de la trazabilidad y no puede modificarse. Antes de esta fecha, `PUT /stock/{id}` permitía cambiar `material_id` en cualquier estado (incluidas chapas `RESERVED`/`CONSUMED`), sin ningún guard — hallazgo de la revisión de deuda técnica P2. Corregido con el mismo patrón de `UPDATE` condicional que ya usan `reserve_stock`/`confirm_cut` (`WHERE status = 'AVAILABLE'`, chequeo de `rowcount`): si la chapa deja de estar `AVAILABLE` justo antes de persistir (carrera con un `reserve`/`discard`/`confirm-cut` concurrente), el `UPDATE` no afecta ninguna fila y se responde `409` sin modificar nada. El movimiento `ADJUSTED` (ya existente, no se agregó ningún tipo nuevo) se sigue logueando, solo que ahora exclusivamente para el caso `AVAILABLE` que es el único que puede ocurrir.

Verificar siempre contra `routes_stock.py` antes de asumir, los permisos por acción no son simétricos.

**Validación de inputs, agregada 2026-09-15 (antes casi no existía a nivel schema)**: `backend/app/schemas/validators.py` centraliza las reglas de texto (`clean_required_text`/`clean_optional_text`: no vacío/no solo espacios, sin control chars, límite de longitud) y las de `Client` (`validate_cuit`, `validate_phone`). Decisiones de negocio confirmadas contra el código/documentación existente, no inventadas: `Client.cuit_cuil` y `Client.phone` **siguen siendo opcionales** (no hay ningún requisito documentado que los exija) pero si se cargan deben tener formato válido (CUIT/CUIL: 11 dígitos ignorando guiones; teléfono: sin letras, al menos 6 dígitos, sin formato internacional rígido). `Client.email` usa `EmailStr` (mismo mecanismo que ya usaba `User.email`), acepta cualquier dominio válido. `Client.name` no admite blanco pero sí números/puntuación (nombres comerciales). `Material.thickness_mm`/`sheet_width_mm`/`sheet_height_mm` deben ser `> 0`; `Material.sheet_cost_ars` debe ser `>= 0` (0 es un valor de negocio real y ya usado deliberadamente en `test_quotation_calculator.py` para aislar el costo de máquina — **no** se exigió `> 0` ahí). `Quotation.issue_date`/`due_date`: año mínimo 2000 (bug real reproducido: un `<input type="date">` de HTML5 permite tipear un año de 1 dígito, backend no validaba nada, terminaba en un PDF mostrando "01/05/1"), y `due_date >= issue_date`. Deliberadamente **no** se exigió `issue_date >= hoy`: hay un flujo legítimo de cargar cotizaciones acordadas verbalmente días atrás, y además rompería si se aplicara también a ediciones de cotizaciones históricas (aunque hoy no existe ningún endpoint de edición de cabecera de cotización, solo creación).
...
**Incidente — área con agujeros calculada mal**: `analyze_dxf()` sumaba el área de los contornos internos (agujeros) en vez de restarla, y las piezas puramente circulares (`CIRCLE`) no contaban para el área en absoluto — desalineaba el costo de material cotizado contra el área real que después se descontaba del stock (que sí usaba `extract_piece_polygon`, correcto desde el principio; el motor de stock/reservas/confirm-cut nunca leyó `Piece.area_mm2`, así que este bug jamás afectó qué chapa se reservaba o qué retazo se generaba — quedó acotado 100% al costeo). El fix se escribió el 25/08 pero quedó sin commitear ese día (ver §20 de esa fecha); recién se commiteó y deployó el **26/08** (`134425d`).

*Corrección sobre una afirmación previa de esta memoria*: acá y en `PROJECT_CONTEXT.md` se había dicho que el 25/08 ya se había aplicado un backfill a las piezas existentes en producción. **Eso era incorrecto** — verificado con `git log`/`git show` en una auditoría de contexto el 2026-09-14: no existía ningún commit, script ni migración de backfill en el repo, y cronológicamente el 25/08 no pudo haberse ejecutado con una fórmula que todavía no estaba deployada.

**Cierre del incidente (P0), 2026-09-14**: se implementó `backend/scripts/recalculate_piece_geometry.py` (commit `a089991`) — script administrativo idempotente, dry-run por defecto (ver tests en `backend/tests/test_recalculate_piece_geometry.py`), con esta política, verificada en código (`QuotationItem` guarda su propio snapshot de costos — `cost_material_ars`/`cost_machine_ars`/`cost_labor_ars`/`unit_price_ars`/`total_price_ars` — calculado una sola vez al crear/editar el ítem, nunca recalculado en un `GET`):
- Cotizaciones `sent`/`accepted`/`cancelled`: **conservan su snapshot de costos intacto, byte a byte** — el script corrige la `Piece`, pero nunca toca esos ítems (no hay ningún camino de código que pueda hacerlo, `_require_editable` lo bloquea).
- Ítems en cotizaciones `draft`: se recalculan automáticamente con `calculate_quotation_item()` real, solo si la geometría de la pieza cambió de verdad y la cotización sigue en `draft` en el momento exacto de persistir (guard contra carrera con un `UPDATE` condicional).

Se corrió el **dry-run real contra la PostgreSQL de producción** (vía `railway run -s Postgres -e production`, sin `--apply`, sin exponer credenciales). Resultado: **6 piezas con `dxf_data` inspeccionadas, las 6 ya tenían `area_mm2`/`length_cut_mm` correctos, 0 piezas requerían corrección, 0 errores de DXF, 0 `QuotationItem` en `draft` requerían recálculo**. Ninguna pieza real en producción tiene agujeros internos ni una entidad `CIRCLE` — el patrón geométrico que dispara el bug no está presente en los datos actuales.

**Se verificó producción mediante dry-run y no se encontraron registros que requirieran corrección — por lo tanto no fue necesario ejecutar `--apply`.** No se aplicó ningún backfill masivo, porque no hacía falta. Incidente cerrado como **verificado sin datos afectados en producción**, no como "backfill aplicado" (nunca se ejecutó `--apply`, no había nada que escribir).

**Piezas legacy sin DXF** (2 de las 8 piezas totales en producción, `dxf_data IS NULL` — creadas antes de que el DXF fuera obligatorio al crear una pieza, ver `54888ef`): quedan **fuera del alcance de este script** porque no existe geometría original a partir de la cual recalcular nada — no hay DXF que reanalizar. No se las marca como incorrectas; son simplemente registros legacy **no verificables por este mecanismo**. Si en algún momento se quiere resolver esto, la única vía es que alguien les suba un DXF real a mano vía `PUT /pieces/{id}/upload-dxf` (mismo endpoint de siempre); no hay forma de inferir su geometría sin el archivo original.

**Estado del script hacia adelante**: `recalculate_piece_geometry.py` queda en el repositorio como herramienta administrativa de uso general — no es un script de un solo uso para este incidente puntual. Sirve para auditoría futura, verificación de datos, e importaciones legacy (por ejemplo si se migra un lote de piezas desde otro sistema). Su modo por defecto sigue siendo dry-run; eso no cambia.
...
**Descartar una chapa — regla final, corregida 2026-09-14 (último P1 de la auditoría final)**: solo se puede descartar (`DISCARDED`) una chapa `AVAILABLE`. `RESERVED`, `CONSUMED` y `DISCARDED` devuelven `409` — **decisión deliberada (Opción A, no automática)**: si una chapa está `RESERVED`, el usuario debe liberar la reserva explícitamente (`release`) antes de poder descartarla, en vez de que `discard` libere la reserva como efecto secundario oculto. Mismo criterio que el fix de `PUT /stock/{id}` (`a7f25ca`): una operación administrativa no debe tener consecuencias silenciosas sobre una reserva/cotización activa, y mantener cada acción con una sola responsabilidad reduce el riesgo de dejar estados intermedios. Antes de esta fecha, `_DISCARDABLE_STATUSES` incluía `RESERVED`, y `discard_stock_sheet` hacía un `SELECT` → chequeo en Python → asignación directa sin `UPDATE` condicional — se podía descartar una chapa `RESERVED` dejando su `StockReservation` `ACTIVE` huérfana (sin ningún camino de UI para liberarla), y bajo una carrera real contra `confirm_cut` era posible sobrescribir `CONSUMED → DISCARDED` **después** de haber generado retazos. Corregido con el mismo patrón de `UPDATE` condicional (`WHERE status = 'AVAILABLE'`, chequeo de `rowcount`) que ya usan `reserve_stock`/`confirm_cut`/`update_stock_sheet` — esto cierra las tres carreras a la vez (discard-vs-reserve, discard-vs-confirm-cut, discard-vs-discard) de forma estructural, no solo por timing: una vez que la chapa deja de ser `AVAILABLE`, este `UPDATE` nunca vuelve a matchearla.
...
**Baja de usuario/empleado — dos conceptos separados, verificado 2026-09-15**: `User.is_active` (global, sin UI para desactivarlo hoy, solo vía DB) bloquea `/auth/login` y `/auth/refresh`. `CompanyMember.is_active` (baja de un empleado dentro de UNA empresa, con UI en "Empleados") NO bloquea el login global a propósito — un usuario puede pertenecer a otras empresas — pero `get_current_company` sí bloquea cualquier endpoint que dependa de una empresa (que es, en la práctica, casi todo el sistema salvo `/auth/*` y `/companies/me`). El bug reportado por una tester ("me deja ingresar con mi usuario dado de baja") no era ninguno de los dos: el backend ya estaba correcto en ambos frentes. Era 100% un problema de UX en `LoginPage.tsx`, que auto-seleccionaba la única empresa del usuario sin mirar `member_is_active`/`is_active`, dándole al empleado desactivado la impresión de haber entrado (para chocar después con 403 en cada pantalla). Corregido filtrando por empresas activas antes de auto-seleccionar/navegar.

**Corrección del mismo día, tras un repaso pidiendo distinguir "revocar token" de "revalidar estado del usuario"**: el frontend fix no alcanzaba. `get_current_user` (en `auth_guard.py`) nunca consultaba la DB — solo decodificaba el JWT — así que un access_token emitido antes de una baja global (`User.is_active=False`) seguía sirviendo para CUALQUIER endpoint protegido (incluido, en cascada, `get_current_company`) hasta que expiraba por su propio `exp`. Solo `/auth/login`/`/auth/refresh` revalidaban. Corregido: `get_current_user` ahora hace `db.query(User).filter(User.id == user_id).first()` y corta con `401` si no existe o `is_active=False`. Esto es explícitamente **revalidación de estado, no revocación de tokens** — no hay blacklist ni versión de JWT (decisión deliberada, no se va a implementar), el token sigue siendo válido criptográficamente, pero ahora se re-chequea contra la DB en cada uso. `CompanyMember.is_active=False` sigue funcionando igual que antes (por diseño): bloquea `get_current_company` para ESA empresa, sin afectar la sesión global del usuario en otras empresas donde sigue activo — ver `test_auth_inactive.py::test_deactivated_membership_in_one_company_does_not_affect_another`. Ver tests en `test_auth_inactive.py`.
