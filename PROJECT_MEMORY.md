...
*Corrección sobre una afirmación previa de esta memoria*: acá y en `PROJECT_CONTEXT.md` se había dicho que el 25/08 ya se había aplicado un backfill a las piezas existentes en producción. **Eso era incorrecto** — verificado con `git log`/`git show` en una auditoría de contexto el 2026-09-14: no existe ningún commit, script ni migración de backfill en el repo, y cronológicamente el 25/08 no pudo haberse ejecutado con una fórmula que todavía no estaba deployada. **El backfill de piezas históricas (con agujeros o geometría circular, creadas antes del 26/08) sigue pendiente de correr contra producción.**

Ya existe el script listo y testeado para hacerlo (`backend/scripts/recalculate_piece_geometry.py`, dry-run por defecto — ver tests en `backend/tests/test_recalculate_piece_geometry.py`), con esta política, verificada en código (`QuotationItem` guarda su propio snapshot de costos — `cost_material_ars`/`cost_machine_ars`/`cost_labor_ars`/`unit_price_ars`/`total_price_ars` — calculado una sola vez al crear/editar el ítem, nunca recalculado en un `GET`):
...
- Ítems en cotizaciones `draft`: **sí se recalculan automáticamente** con `calculate_quotation_item()` real cuando se ejecute el backfill, pero solo si la geometría de la pieza cambió de verdad y la cotización sigue en `draft` en el momento exacto de persistir (guard contra carrera con un `UPDATE` condicional).

**Todavía no se ejecutó `--apply` contra la base real.** No asumir que los datos de producción ya están corregidos hasta que se corra y se verifique explícitamente.
